...
it.layer(layer)("AntigravityAdapter", (it) => {
  it.effect("keeps managed preview instructions scoped to the requesting thread", () =>
    Effect.gen(function* () {
      const h = yield* makeHarness();
      McpProviderSession.setMcpProviderSession({
        environmentId: EnvironmentId.make("preview-test"),
        threadId,
        providerSessionId: "native-preview-test",
        providerInstanceId: instanceId,
        endpoint: "http://127.0.0.1:4312/mcp",
        authorizationHeader: "Bearer test-preview",
      });
      yield* Effect.addFinalizer(() =>
      ...
      const sending = yield* h.adapter
        .sendTurn({ threadId, input: "start dev server" })
        .pipe(Effect.forkChild);
      const prompt = yield* h.nextPrompt;
      expect(
        prompt.content.some(
          (part) => part.type === "text" && part.text.includes("dev_server_ensure"),
        ),
      ).toBe(true);
      yield* Deferred.succeed(prompt.result, { stopReason: "end_turn" });
      yield* Fiber.join(sending);
      McpProviderSession.clearMcpProviderSession(threadId);
      const plainSending = yield* h.adapter
        .sendTurn({ threadId, input: "continue" })
        .pipe(Effect.forkChild);
      const plain = yield* h.nextPrompt;
      expect(
        plain.content.some(
          (part) => part.type === "text" && part.text.includes("dev_server_ensure"),
        ),
      ).toBe(false);
      yield* Deferred.succeed(plain.result, { stopReason: "end_turn" });
      yield* Fiber.join(plainSending);
    }),
  );
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(outsideFile, "private");
      const link = path.join(cwd, "linked.txt");
      yield* fs.symlink(outsideFile, link);
      expect((yield* read({ sessionId: nativeSessionId, path: link }).pipe(Effect.flip))._tag).toBe(
        "AcpRequestError",
      );
      expect(
      ...
      expect(yield* fs.readFileString(outsideFile)).toBe("private");
