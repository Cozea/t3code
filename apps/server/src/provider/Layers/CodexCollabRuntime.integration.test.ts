...
describe("CodexSessionRuntime collab integration", () => {
  it.effect("resumes persisted completed sub-agent history after process replacement", () =>
    Effect.gen(function* () {
      const temporary = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "cozea-codex-resume-"));
      const savedScript = NodePath.join(temporary, "script.json");
      ...
      const options = {
        threadId: ThreadId.make("persisted-history"),
        binaryPath: peerPath,
        cwd: temporary,
        runtimeMode: "full-access" as const,
        environment: { ...process.env, T3_CODEX_COLLAB_SCRIPT: savedScript },
      };
      const firstScope = yield* Scope.fork(yield* Scope.Scope);
      const first = yield* makeCodexSessionRuntime(options).pipe(
        Effect.provideService(Scope.Scope, firstScope),
      );
      const firstSession = yield* first.start();
      const complete = yield* first.events.pipe(
        Stream.filter((event) => event.method === "turn/completed"),
        Stream.take(1),
        Stream.runCollect,
        Effect.forkScoped,
      );
      yield* first.sendTurn({ input: "finish a child" });
      yield* Fiber.join(complete);
      yield* first.close;
      const replacement = yield* makeCodexSessionRuntime({
        ...options,
        resumeCursor: { threadId: ROOT },
      });
      const resumed = yield* replacement.start();
      assert.deepEqual(resumed.resumeCursor, firstSession.resumeCursor);
      const nextComplete = yield* replacement.events.pipe(
        Stream.filter((event) => event.method === "turn/completed"),
        Stream.take(1),
        Stream.runCollect,
        Effect.forkScoped,
      );
      yield* replacement.sendTurn({ input: "continue the same conversation" });
      assert.lengthOf(Array.from(yield* Fiber.join(nextComplete)), 1);
      assert.include(
        NodeFS.readFileSync(`${savedScript}.requests`, "utf8"),
        `"threadId":"${ROOT}"`,
      );
