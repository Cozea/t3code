...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Canonicalize the existing ancestor, including the leaf when it exists.
    // New nested paths must retain macOS /var -> /private/var normalization;
    // existing leaf symlinks must not bypass the session-root boundary.
            ...
            error.reason._tag === "NotFound"
              ...
              : Effect.fail(
                  EffectAcpErrors.AcpRequestError.invalidParams(
                    "Cannot resolve the requested file path.",
                  ),
                ),
          ),
        );
      if (canonical !== undefined) {
        real = path.join(canonical, ...missingSegments);
        break;
      }
